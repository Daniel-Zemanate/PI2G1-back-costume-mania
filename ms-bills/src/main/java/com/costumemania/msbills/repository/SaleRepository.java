...
public interface SaleRepository extends JpaRepository<Sale, Integer> {
    ...
    @Query(value = "SELECT s.id_sale, s.no_invoice, s.users, s.catalog, s.quantity, s.shipping_address, s.shipping_city, s.status, s.sale_date, s.shipping_date , u.username, u.status as statusUser FROM sale s inner join users u ON s.users=u.id_user where s.users=?1", nativeQuery = true)
    Optional<List<Sale>> getByUser (Integer idUser);

    @Query(value = "SELECT s.id_sale, s.no_invoice, s.users, s.catalog, s.quantity, s.shipping_address, s.shipping_city, s.status, s.sale_date, s.shipping_date , c.model, c.size, c.price FROM sale s inner join catalog c on s.catalog=c.id_catalog where c.model=?1", nativeQuery = true)
    Optional<List<Sale>> getByModel (Integer idModel);
    Optional<List<Sale>> getByCatalog (Catalog catalog);
    @Query(value = "SELECT s.id_sale, s.no_invoice, s.users, s.catalog, s.quantity, s.shipping_address, s.shipping_city, s.status, s.sale_date, s.shipping_date , c.model, c.size, c.price FROM sale s inner join catalog c on s.catalog=c.id_catalog inner join size si on c.size=si.id_size where si.adult=?1", nativeQuery = true)
